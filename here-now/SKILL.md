...
## Automated page maintenance (cron pattern)

For pages that display live system state (skill counts, cron jobs, service status, dashboards), set up a cron job with a pre-run script that detects drift and republishes.

**Architecture:**
1. Pre-run script fetches the live page, extracts displayed metrics, compares to actual system state
2. Script outputs JSON: `{"needs_update": bool, "discrepancies": [...]}`
3. Cron agent reads the diff — if `needs_update` is false, reports "up to date" and stops
4. If stale, agent regenerates the HTML and republishes with `--slug {slug}`

**Pre-run script checklist:**
- Use `urllib.request` (or similar stdlib) to fetch the page HTML — agent tools aren't loaded yet
- Parse metrics from HTML with regex or a small parser
- Get live data via whatever your source of truth is (a CLI, an API, a config file)
- For comparing display names vs registry names, use fuzzy matching: normalize + similarity ratio + word-level overlap
- Output JSON to stdout — the cron agent receives it as context

**Pitfalls:**
- Don't count entities via filesystem walk if your runtime has duplicates or backups — use the canonical CLI
- Make sure the publish script's credentials file is readable in the cron environment
- Compare actual displayed text, not just file existence — pages can become stale without files changing
